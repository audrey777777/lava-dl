...
class SpikeMax(torch.nn.Module):
    ...
    reduction : str
        loss reduction method. One of 'sum'|'mean'. Defaults to 'sum'.
    alpha : int
        Softmax temperature parameter. Logit z = r / alpha. Only valid
        when mode is 'softmax'. Defaults to 1.
    theta : int
        Bias term for logits. Logit z = (r - theta) / alpha. Only valid
        when mode is 'softmax'. Defaults to 1.
    """
    def __init__(
        self, moving_window=None, mode='probability', reduction='sum', alpha=1,
        theta=0,
    ):
    ...
    def forward(self, input, label):
        ...
        if self.window is None:  # one label for each sample in a batch
            ...
            if self.mode == 'probability':
                if self.alpha != 1 and self.theta:
                    raise AssertionError(
                        f'Cannot specify alpha, theta with mode "probability"'
                    )
                log_p = torch.log(Rate.confidence(input, mode=self.mode))
            else:
                log_p = Rate.confidence(scaled_input, mode='logsoftmax')
            ...
            if self.mode == 'probability':
                ...
                scaled_rates = (rates - self.theta) / self.alpha
                log_p = F.log_softmax(scaled_rates, 1)
            return F.nll_loss(
